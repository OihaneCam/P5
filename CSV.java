...
public class CSV{ 
	...
	private static void procesaLineaDatos( ArrayList<Object> datos ) {
		...
		UsuarioTwitter u = new UsuarioTwitter(
				...
				getLong(datos, 7),
				getString(datos, 8),
				getArrayList(datos,9)		
				);
		GestionTwitter.cargarMapaId(u.getId(), u);
		GestionTwitter.cargaMapaNick(u.getScreenName(), u);
		GestionTwitter.cargaTree(u);
	}
	
	private static String getString(ArrayList<Object> datos, int index ) {
		if(index >= 0 && index < datos.size()) {
			Object obj = datos.get(index);
	...
	private static ArrayList<String> getArrayList(ArrayList<Object> datos, int index){
		if(index >= 0 && index < datos.size()) {
			Object obj = datos.get(index);
	...
	private static Long getLong(ArrayList<Object> datos, int index) {
		if(index >= 0 && index < datos.size()) {
			Object obj = datos.get(index);
